...
`})}),I(`${s.sprint.replace(/\s+/g,"-")}-retro.md`,e,"text/markdown")};window.exportJSON=function(){I(`${s.sprint.replace(/\s+/g,"-")}-retro.json`,JSON.stringify({sprint:s.sprint,demos:s.demos},null,2),"application/json")};function j(){w(a(l,"sprint"),e=>{s.sprint=e.val()||"Sprint 1",document.getElementById("h-sprint").textContent=s.sprint;const t=document.getElementById("cfg-sprint");t&&document.activeElement!==t&&(t.value=s.sprint)}),w(a(l,"team"),e=>{s.team=e.val()?Object.values(e.val()):[],S(),_(),T()}),w(a(l,"demos"),e=>{const t=e.val()||{};s.demos=Object.entries(t).map(([n,i])=>({id:n,presenter:i.presenter||"",topic:i.topic||"",questions:i.questions?Object.entries(i.questions).map(([o,r])=>({id:o,author:r.author||"",text:r.text||"",votes:r.votes||0,intent:r.intent||null,answer:r.answer||"",discussFlaggers:r.discussFlaggers?Object.values(r.discussFlaggers):[],replies:r.replies?Object.entries(r.replies).map(([d,m])=>({id:d,author:m.author||"",text:m.text||""})):[],voters:r.voters||{}})):[]})),v(),P(),F()})}function S(){const e=document.getElementById("team-grid");if(e){if(!s.team.length){e.innerHTML='<div style="color:var(--muted);font-style:italic;font-size:.85rem;grid-column:1/-1">No members.</div>';return}e.innerHTML=s.team.map(t=>`
    ...
    </div>`}).join("")}function R(e){const t=[...e.questions].sort((n,i)=>i.votes-n.votes);return t.length?`<div class="q-list">${t.map(n=>q(n,e)).join("")}</div>`:'<div class="empty-q">No questions yet.</div>'}function q(e,t){const n=btoa(s.user).replace(/=/g,""),i=(e.voters&&e.voters[n])===!0,o=s.user===t.presenter,r=e.intent==="meeting"?'<span class="q-intent meeting">🎙 Explain in meeting</span>':e.intent==="written"?'<span class="q-intent written">✏ Written answer</span>':"",d=e.intent==="written"&&e.answer?`<div class="q-answer-block"><div class="q-answer-label">Answer</div><div class="q-answer-text">${c(e.answer)}</div></div>`:"",m=o?`
